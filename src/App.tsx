...
const LANDING_RECENT_PLACEHOLDERS = ['Kitchen', 'Bathroom', 'Living space'] as const
...
function getKitchenCategoryOption(category: KitchenScoredCategory, value: string | undefined) {
  ...
  if (!value) return { score: 1, adders: [0, 0] as const }
...
function getAnsweredAdderValue(
  map: Record<string, readonly [number, number]>,
  key: string | undefined,
  fallback: string
) {
  if (!key) return [0, 0] as const
...
function getAnsweredAdditionValue<T extends Record<string, readonly [number, number]>>(map: T, key: string | undefined, fallback: keyof T) {
  if (!key) return [0, 0] as const
  if (map[key as keyof T]) return map[key as keyof T]
  return map[fallback]
}
...
function calculateEstimate(project: Project | undefined, tier: string, answers: Record<string, string>) {
  ...
  if (project.id === 'kitchen') {
    ...
    const sizeKey = answers.size as keyof typeof kitchenPricing.sizeAdders | undefined
    const [sizeLowAdder, sizeHighAdder] = sizeKey ? (kitchenPricing.sizeAdders[sizeKey] || kitchenPricing.sizeAdders.not_sure) : [0, 0]
  ...
  if (project.id === 'bathroom') {
    const [typeLow, typeHigh] = getAnsweredAdderValue(bathroomPricing.typeAdders, answers.bathroomType, 'full_bathroom')
    const [sizeLow, sizeHigh] = getAnsweredAdderValue(bathroomPricing.sizeAdders, answers.bathroomSize, '40_80')
    const [layoutLow, layoutHigh] = getAnsweredAdderValue(bathroomPricing.layoutAdders, answers.bathLayout, 'none')
    const [showerLow, showerHigh] = getAnsweredAdderValue(bathroomPricing.showerTubAdders, answers.showerTub, 'refresh')
    const [vanityLow, vanityHigh] = getAnsweredAdderValue(bathroomPricing.vanityAdders, answers.vanity, 'basic_prefab')
    const [tileLow, tileHigh] = getAnsweredAdderValue(bathroomPricing.tileAdders, answers.tile, 'minimal')
    const [finishLow, finishHigh] = getAnsweredAdderValue(bathroomPricing.finishAdders, answers.finishLevel, 'standard')
  ...
  if (project.id === 'suite') {
    const [sizeLow, sizeHigh] = getAnsweredAdderValue(suitePricing.sizeAdders, answers.suiteSize, 'not_sure')
    const [projectLow, projectHigh] = getAnsweredAdderValue(suitePricing.projectTypeAdders, answers.suiteProjectType, 'not_sure')
    const [finishLow, finishHigh] = getAnsweredAdderValue(suitePricing.finishAdders, answers.suiteFinish, 'not_sure')
    const [siteLow, siteHigh] = getAnsweredAdderValue(suitePricing.siteComplexityAdders, answers.suiteSiteComplexity, 'not_sure')
    const [kitchenLow, kitchenHigh] = getAnsweredAdderValue(suitePricing.kitchenAdders, answers.suiteKitchen, 'not_sure')
    const [bathLow, bathHigh] = getAnsweredAdderValue(suitePricing.bathroomAdders, answers.suiteBathroom, 'not_sure')
    const [utilitiesLow, utilitiesHigh] = getAnsweredAdderValue(suitePricing.utilitiesAdders, answers.suiteUtilities, 'not_sure')
    const finalLow = roundPresentation(
  ...
  if (project.id === 'addition') {
    ...
    const sizeKey = answers.additionSize as keyof typeof additionPricing.defaultSqftBySize | undefined
    const sizeWeight = sizeKey
      ? (additionPricing.defaultSqftBySize[sizeKey] || additionPricing.defaultSqftBySize.not_sure) / additionPricing.defaultSqftBySize.not_sure
      : 0
    const [baseTypeLow, baseTypeHigh] = answers.additionType
      ? getAdditionValue(additionPricing.typeAddersBySize, resolvedAdditionType, 'not_sure')
      : ([0, 0] as const)
    const [finishLow, finishHigh] = getAnsweredAdditionValue(additionPricing.finishAdders, answers.finishLevel, 'mid_range')
    ...
    const [plumbingMin, plumbingMax] = getAnsweredAdditionValue(additionPricing.plumbingAdjustments, answers.plumbingScope, 'not_sure')
    const [hvacMin, hvacMax] = getAnsweredAdditionValue(additionPricing.hvacAdjustments, answers.hvacScope, 'not_sure')
    const [structuralMin, structuralMax] = getAnsweredAdditionValue(additionPricing.structuralAdjustments, answers.structuralScope, 'not_sure')
    const [exteriorMin, exteriorMax] = getAnsweredAdditionValue(additionPricing.exteriorAdjustments, answers.exteriorScope, 'not_sure')
    const [matchMin, matchMax] = getAnsweredAdditionValue(additionPricing.matchAdjustments, answers.matchQuality, 'not_sure')
    const [permitMin, permitMax] = getAnsweredAdditionValue(additionPricing.permitDesignAdjustments, answers.permitDesignScope, 'not_sure')
    ...
    const [siteMin, siteMax] = answers.siteDifficulty
      ? getAdditionValue(additionPricing.siteDifficultyMultiplier, answers.siteDifficulty, 'not_sure')
      : ([1, 1] as const)
      ...
      <section className="project-preview" aria-labelledby="projects-heading">
            ...
            return (
              ...
              >
                ...
                <span className="landing-project-copy">
                  ...
                  <span className="landing-project-meta">Photo pending approval</span>
                </span>
              </button>
            )
          })}
        ...
        <section className="recent-work" aria-labelledby="recent-work-heading">
          <div className="recent-work-heading">
            ...
            <span className="landing-micro">Photos withheld pending approval</span>
          </div>
          <div className="recent-work-grid">
            {LANDING_RECENT_PLACEHOLDERS.map((label) => (
              <figure key={label} className="recent-work-item">
