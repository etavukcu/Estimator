...
const shouldUseServerApi = Boolean(apiBaseUrl)
...
async function postToServerApi(payload: Record<string, unknown>) {
  if (!shouldUseServerApi) return null
...
export async function insertConsultationRequest(payload: Record<string, unknown>) {
  ...
  throw new Error('Missing consultation submit configuration. Set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY (or set VITE_API_BASE_URL to a live API server).')
}
