...
export async function insertConsultationRequest(payload: Record<string, unknown>) {
  ...
  throw new Error('Consultation submit endpoint is unavailable. Deploy /api/consultation-requests (or set VITE_API_BASE_URL), or configure VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY for direct fallback.')
}
